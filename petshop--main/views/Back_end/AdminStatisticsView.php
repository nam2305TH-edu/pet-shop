            ...
            <form method="GET" class="row g-3 align-items-end">
                ...
                <div class="col-md-3">
                    ...
                    <select name="filterType" id="filterType" class="form-select" onchange="toggleFilterInputs()">
                        ...
                        <option value="range" <?= $data['filterType'] === 'range' ? 'selected' : '' ?>>Khoảng ngày</option>
                    </select>
                </div>
                ...
                <div class="col-md-3" id="filterRangeWrapper" style="display: <?= $data['filterType']==='range' ? 'block' : 'none' ?>;">
                    <label class="form-label">Từ ngày - đến ngày</label>
                    <div class="d-flex gap-2">
                        <input type="date" name="from" class="form-control" value="<?= htmlspecialchars($data['from'] ?? '') ?>">
                        <input type="date" name="to" class="form-control" value="<?= htmlspecialchars($data['to'] ?? '') ?>">
                    </div>
                </div>
                ...
                <div class="col-md-12 d-flex gap-2 flex-wrap">
                    ...
                    <?php if ($data['filterType'] !== 'all'): ?>
                        <?php if ($data['filterType'] === 'range'): ?>
                        <span class="badge bg-info text-dark align-self-center">Đang lọc: từ <?= htmlspecialchars($data['from'] ?? '') ?> đến <?= htmlspecialchars($data['to'] ?? '') ?></span>
                        <?php else: ?>
...
function toggleFilterInputs() {
    ...
    const range = document.getElementById('filterRangeWrapper');
    ...
    if (range) range.style.display = type === 'range' ? 'block' : 'none';
}
