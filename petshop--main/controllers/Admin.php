...
class Admin extends Controller{
    ...
    public function statistics()
    {
        ...
        // Support range filter: from-to dates
    ...
    // 👥 Quản lý khách hàng: xem thông tin, số đơn mua, tổng tiền
    public function customers()
    {
        // Lấy thống kê theo buyer từ AdminStatisticsModel
        $statsModel = $this->model('AdminStatisticsModel');
        $buyersStats = $statsModel->getStatsByBuyer('all', null); // user_email, fullname, revenue, order_count

        // Map theo email để bổ sung thông tin từ bảng người dùng
        $userModel = $this->model('UserModel');
        $users = $userModel->all('tbluser');
        $userByEmail = [];
        foreach ($users as $u) {
