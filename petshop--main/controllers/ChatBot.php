...
class ChatBot extends Controller {
    ...
    private function callOllama($message) {
        ...
        // Nạp thêm dữ liệu huấn luyện từ file JSON nếu có
        // Hỗ trợ nạp nhiều nguồn dữ liệu JSON
        $dataFiles = [
            __DIR__ . '/../data/chatbot_faq.json',
            __DIR__ . '/../data/chatbot_faq_extra.json'
        ];
        foreach ($dataFiles as $faqFile) {
            if (file_exists($faqFile)) {
                try {
                    $json = file_get_contents($faqFile);
                    $extra = json_decode($json, true);
                    if (is_array($extra)) {
                        $faqData = array_merge($faqData, $extra);
                    }
                } catch (Throwable $e) {}
            }
        }
        ...
        // Tìm câu trả lời: hỗ trợ khớp theo từ khóa, regex và chuẩn hóa dấu
        foreach ($faqData as $faq) {
            $keywords = $faq['keywords'] ?? [];
            $regexes = $faq['regex'] ?? [];
            $reply = $faq['reply'] ?? '';
            foreach ($keywords as $keyword) {
                $kwNorm = $this->normalizeText(strtolower($keyword));
        ...
        $defaultReplies = [
            ...
            '📞 Nếu bạn cần trợ giúp ngay, hãy liên hệ hotline: 037993243'
        ];
        
        return $defaultReplies[array_rand($defaultReplies)];
    }

    // Chuẩn hóa văn bản tiếng Việt: bỏ dấu, loại bỏ ký tự thừa
    private function normalizeText($text) {
        $text = strtolower(trim($text));
        // Bỏ dấu tiếng Việt
        $search = ['à','á','ả','ã','ạ','ă','ằ','ắ','ẳ','ẵ','ặ','â','ầ','ấ','ẩ','ẫ','ậ',
        ...
        // Loại bỏ ký tự không chữ số/chữ cái cơ bản
