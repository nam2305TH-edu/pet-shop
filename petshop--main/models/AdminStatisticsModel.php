...
class AdminStatisticsModel extends BaseModel {
    ...
    public function getTotalRevenue($filterType = 'all', $filterValue = null) {
        ...
        } elseif ($filterType === 'range' && is_array($filterValue) && !empty($filterValue[0]) && !empty($filterValue[1])) {
            $query .= " AND DATE(created_at) BETWEEN :from AND :to";
        }
        
        $stmt = $this->db->prepare($query);
        if ($filterType === 'range' && is_array($filterValue)) {
            $stmt->bindParam(':from', $filterValue[0]);
            $stmt->bindParam(':to', $filterValue[1]);
        } elseif ($filterValue) {
    ...
    // Top 10 distributor (doanh thu cao nhất - chỉ tính đơn đã nhận hàng)
        ...
        if ($filterType === 'day' && $filterValue) {
            ...
            $query .= " AND DATE(o.created_at) BETWEEN :from AND :to";
        }
    ...
    public function getFinancialSummary($filterType = 'all', $filterValue = null) {
        ...
        if ($filterType === 'day' && $filterValue) {
            ...
            $revQuery .= " AND DATE(o.created_at) BETWEEN :from AND :to";
            $costQuery .= " AND DATE(o.created_at) BETWEEN :from AND :to";
        }
        ...
        if ($filterType === 'range' && is_array($filterValue)) {
            $stmt->bindParam(':from', $filterValue[0]);
            $stmt->bindParam(':to', $filterValue[1]);
        } elseif ($filterValue) { $stmt->bindParam(':filterValue', $filterValue); }
        $stmt->execute();
        $row = $stmt->fetch();
